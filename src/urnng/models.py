...
class RNNLM(nn.Module):
    def __init__(self, vocab=10000, w_dim=650, h_dim=650, num_layers=2, dropout=0.5):
        ...
        self.rnn = nn.LSTM(
            w_dim, h_dim, num_layers=num_layers, dropout=dropout, batch_first=True
        )
    ...
    def generate(self, bos=2, eos=3, max_len=150):
        ...
        bos = torch.LongTensor(1, 1).to(device).fill_(bos)
...
class SeqLSTM(nn.Module):
    def __init__(self, i_dim=200, h_dim=0, num_layers=1, dropout=0):
        ...
        self.linears = nn.ModuleList(
            [
                nn.Linear(h_dim + i_dim, h_dim * 4)
                if l == 0
                else nn.Linear(h_dim * 2, h_dim * 4)
    ...
    def forward(self, x, prev_h=None):
        if prev_h is None:
            prev_h = [
                (
                    x.new(x.size(0), self.h_dim).fill_(0),
                    x.new(x.size(0), self.h_dim).fill_(0),
                )
...
class RNNG(nn.Module):
    ...
    def get_span_scores(self, x):
        ...
        pos = torch.arange(0, x.size(1)).unsqueeze(0).expand_as(x).long().to(device)
    ...
    def forward(self, x, samples=1, is_temp=1.0, has_eos=True):
        ...
        crf_input = scores.unsqueeze(1).expand(
            batch, samples, parse_length, parse_length
        )
        crf_input = crf_input.contiguous().view(
            batch * samples, parse_length, parse_length
        )
        ...
        _, log_probs_action_q, tree_brackets, spans = self.q_crf._sample(
            crf_input, self.q_crf.alpha
        )
        actions = []
        ...
        for b in range(batch_expand):
            stack2[b].append(
                [
                    [init_stack[l][0][b], init_stack[l][1][b]]
                    for l in range(self.num_layers)
                ]
            )
        ...
        for l in range(num_action):
            ...
            for b in range(batch_expand):
                ...
                if actions[b][l].item() == self.S:
                    ...
                    stack_child[b].append(
                        (word_vecs[b][pointer[b]], word_vecs_zeros[b][pointer[b]])
                    )
                    pointer[b] += 1
                else:
                    input_b = new_child[0][child_idx].unsqueeze(0)
                    stack_child[b].append(
                        (input_b, new_child[1][child_idx].unsqueeze(0))
                    )
                    child_idx += 1
            ...
            for k in range(self.num_layers):
                stack_h_all.append(
                    (torch.stack(stack_h[k][0], 0), torch.stack(stack_h[k][1], 0))
                )
        ...
        action_score = (
            1 - actions
        ) * action_shift_score + actions * action_reduce_score
        action_score = (action_score * action_masks).sum(1)

        word_contexts = contexts[actions < 1]
        word_contexts = word_contexts.contiguous().view(
            batch * samples, length, self.h_dim
        )

        log_probs_word = F.log_softmax(self.vocab_mlp(word_contexts), 2)
        log_probs_word = torch.gather(log_probs_word, 2, x_expand.unsqueeze(2)).squeeze(
            2
        )
    ...
    def forward_tree(self, x, actions, has_eos=True):
        ...
        for b in range(batch):
            ...
            gold_spans[b].copy_(torch.eye(length).to(device))
