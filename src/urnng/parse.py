...
parser.add_argument("--out_file", default="pred-parse.jsonl")
...
def main(args):
    ...
    with torch.no_grad():
        for j, gold_tree in enumerate(open(args.data_file, "r")):
            ...
            # This is unused but raise an IndexError for KTB.
            ...
            sent_idx = [word2idx["<s>"]] + [
                word2idx[w] if w in word2idx else word2idx["<unk>"] for w in sent
            ]
            ...
            (
                ...
                actions_all,
                q_entropy,
            ) = model(sents, samples=1, is_temp=1, has_eos=False)
            ...
            pred_span_set = set(
                pred_span[:-1]
            )  # the last span in the list is always the

            print("pred_span:", pred_span)
            pred_str_l.append(json.dumps(pred_span))

            # gold_span_set = set(
            #     gold_span[:-1]
            # )  # trival sent-level span so we ignore it
