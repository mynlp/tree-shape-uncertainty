...
from diora.data.dataset import (
    ConsolidateDatasets,
    ReconstructDataset,
    make_batch_iterator,
)
...
from diora.utils.flags import init_with_flags_file, save_flags, stringify_flags
from diora.utils.path import package_path

# Used to check if train loss converges.
EPS = 1e-4
...
# Added
def calc_val_loss(trainer, validation_iterator):
    batches = validation_iterator.get_iterator()

    num_batch = 0
    total_loss = 0.0

    for batch_map in batches:
        results = trainer.step(batch_map, train=False, compute_loss=True)
...
def run_train(options, train_iterator, trainer, validation_iterator):
    ...
    for epoch, seed in zip(range(options.max_epoch), seeds):
        ...
        for batch_idx, batch_map in myiterator():
            ...
            # Added
            epoch_train_loss += result["total_loss"]
            num_batch += 1

            experiment_logger.record(result)

            if step % options.log_every_batch == 0:
                experiment_logger.log_batch(
                    epoch, step, batch_idx, batch_size=options.batch_size
                )
            ...
            #        trainer.save_model(
            #            os.path.join(options.experiment_path, "model_periodic.pt")
            #        )
            #        save_experiment(
            #            os.path.join(
            #                options.experiment_path, "experiment_periodic.json"
            #            ),
            #            step,
            #        )
            ...
            #        trainer.save_model(
            #            os.path.join(
            #                options.experiment_path, "model.step_{}.pt".format(step)
            #            )
            #        )
            #        save_experiment(
            #            os.path.join(
        ...
        val_loss = calc_val_loss(
            trainer=trainer, validation_iterator=validation_iterator
        )

        if (best_val_loss is None) or (val_loss <= best_val_loss):
            # Save the best val_loss model.
            logger.info("Saving the best model.")
            trainer.save_model(os.path.join(options.experiment_path, "model.pt"))
            save_experiment(
                os.path.join(
                    options.experiment_path,
                    "experiment.json",
        ...
        logger.info(
            "Epoch: {}, Loss: {}, Val loss: {}, Best val loss: {}".format(
                epoch, cur_epoch_train_loss, val_loss, best_val_loss
            )
        )
...
def get_train_dataset(options):
    return ReconstructDataset().initialize(
        options,
        text_path=options.train_path,
        # embeddings_path=options.embeddings_path,
...
def get_validation_dataset(options, train_word2idx, train_embedding):
    return ReconstructDataset().initialize(
        options,
        ...
        ext_word2idx=train_word2idx,
        ext_embeddings=train_embedding,
    )
...
def get_test_dataset(options, train_word2idx, train_embedding):
    return ReconstructDataset().initialize(
        options,
        text_path=options.test_path,
        # embeddings_path=options.embeddings_path,
        filter_length=options.test_filter_length,
        data_type=options.test_data_type,
        ext_word2idx=train_word2idx,
        ext_embeddings=train_embedding,
    )


def get_test_iterator(options, dataset):
    return make_batch_iterator(
        options,
        dataset,
        shuffle=False,
...
# Added
# Note that test dataset is necessary to use one-hot embedding consistently during test.
# def get_train_validation_and_test(options):
#    train_dataset = get_train_dataset(options)
#    validation_dataset = get_validation_dataset(options)
#    test_dataset = get_test_dataset(options)
#
#    # Modifies datasets. Unifying word mappings, embeddings, etc.
#    ConsolidateDatasets([train_dataset, validation_dataset, test_dataset]).run()
#
...
def run(options):
    ...
    validation_dataset = get_validation_dataset(
        options,
        train_word2idx=train_dataset["word2idx"],
        train_embedding=train_dataset["embeddings"],
    )
    ...
    # validation_iterator is not used in buildnet anyway.
    trainer = build_net(options, embeddings)

    # don't know why, but this is necessary...
    trainer.net = trainer.net.float()
...
def argument_parser():
    ...
    parser.add_argument(
        "--flags", default=None, type=str, help="Load any settings from a flags file."
    )
    ...
    parser.add_argument(
        ...
        default=os.path.join(package_path(), "..", "log"),
        type=str,
    )
    ...
    parser.add_argument(
        "--validation_data_type", default=None, choices=data_types_choices
    )
    parser.add_argument("--test_data_type", default=None, choices=data_types_choices)

    parser.add_argument(
        "--train_path",
        default=os.path.expanduser("~/data/snli_1.0/snli_1.0_train.jsonl"),
        type=str,
    )
    parser.add_argument(
        "--validation_path",
        default=os.path.expanduser("~/data/snli_1.0/snli_1.0_dev.jsonl"),
        type=str,
    )
    parser.add_argument(
        "--test_path",
    ...
    parser.add_argument(
        "--embeddings_path",
        default=os.path.expanduser("~/data/glove/glove.6B.300d.txt"),
        type=str,
    )
    ...
    parser.add_argument("--test_filter_length", default=0, type=int)

    # Model.
    parser.add_argument(
        "--arch", default="treelstm", choices=("treelstm", "mlp", "mlp-shared")
    )
    ...
    # Model (Objective).
    parser.add_argument(
        "--reconstruct_mode", default="margin", choices=("margin", "softmax")
    )
    ...
    # Add onehot
    parser.add_argument(
        "--emb", default="w2v", choices=("w2v", "elmo", "both", "onehot")
    )
    ...
    parser.add_argument(
        "--elmo_options_path",
        default="https://s3-us-west-2.amazonaws.com/allennlp/models/elmo/2x4096_512_2048cnn_2xhighway/elmo_2x4096_512_2048cnn_2xhighway_options.json",
        type=str,
    )
    parser.add_argument(
        "--elmo_weights_path",
        default="https://s3-us-west-2.amazonaws.com/allennlp/models/elmo/2x4096_512_2048cnn_2xhighway/elmo_2x4096_512_2048cnn_2xhighway_weights.hdf5",
        type=str,
    )
    parser.add_argument(
        "--elmo_cache_dir",
    ...
    parser.add_argument(
        "--retain_file_order", action="store_true"
    )  # If true, then outputs are written in same order as read from file.
...
def parse_args(parser):
    ...
    options.train_data_type = (
        options.data_type
        if options.train_data_type is None
        else options.train_data_type
    )
    options.validation_data_type = (
        options.data_type
        if options.validation_data_type is None
        else options.validation_data_type
    )

    # Added
    options.test_data_type = (
        options.data_type if options.test_data_type is None else options.test_data_type
    )

    options.validation_batch_size = (
        options.batch_size
        if options.validation_batch_size is None
        else options.validation_batch_size
    )

    options.test_batch_size = (
        options.batch_size
        if options.test_batch_size is None
        else options.test_batch_size
    )

    # Set default flag values (config).
    if not options.git_branch_name:
        options.git_branch_name = (
    ...
    if not options.git_dirty:
        options.git_dirty = (
            os.popen("git diff --quiet && echo 'clean' || echo 'dirty'").read().strip()
        )
    ...
    if not options.experiment_path:
        options.experiment_path = os.path.join(
            options.default_experiment_directory, options.experiment_name
        )
