...
def postprocess(tr, tokens=None):
    ...
    assert (
        len(kept) == len(tokens) - 1
    ), "Incorrect tokens left. Original = {}, Output = {}, Kept = {}".format(
        binary_tree, tr, kept
    )
    ...
    def finish(self):
        if not self.retain_file_order:
            ...
            with open(self.output_file, "w") as f:
                for idx, o in sorted(
                    zip(self.state["file_order"], self.state["o"]), key=lambda x: x[0]
                ):
...
def run(options):
    ...
    test_dataset = get_test_dataset(
        options, train_word2idx=word2idx, train_embedding=embeddings
    )

    # Note that unkown tokens cannot be recovered.
    idx2word = {v: k for k, v in word2idx.items()}

    logger.info("Initializing model.")
    # trainer = build_net(options, embeddings, validation_iterator)

    # validation_iterator is not used in build_net.
    trainer = build_net(options, embeddings)

    test_iterator = get_test_iterator(options, test_dataset)
    ...
    batches = test_iterator.get_iterator(random_seed=options.seed)
    ...
    file_writer = FileWriter(
        ...
        retain_file_order=options.retain_file_order,
        idx2word=idx2word,
    )
