...
import numpy as np
import torch
from diora.external.standalone_elmo import (
...
class ElmoEmbedder(object):
    ...
    def __call__(self, word2idx):
        ...
        if os.path.exists(embeddings_file):
            ...
            logger.info(
                "Begin caching vectors. shape = {}, cuda = {}".format(shape, self.cuda)
            )
            ...
            for start in tqdm(range(0, len(tokens), batch_size), desc="embed"):
                ...
                vec = remove_sentence_boundaries(
                    output["token_embedding"], output["mask"]
                )[0].squeeze(1)
        ...
        index = [
            sorted_word2idx[tok]
            for tok, idx in sorted(word2idx.items(), key=sort_by_idx)
        ]
...
def get_onehot(word2idx):
    vocab_size = len(word2idx.keys())
    embeddings = np.identity(vocab_size)
...
class EmbeddingsReader(object):
    ...
    def get_emb_both(self, options, embeddings_path, word2idx):
        ...
        vocab = [
            w for w, i in sorted(w2i_w2v.items(), key=lambda x: x[1]) if w in w2i_elmo
        ]
    ...
    def get_onehot(self, word2idx):
        # Add unk to word2idx.
        if UNK_TOKEN in word2idx:
            raise Exception("UNK_TOKEN already exists.")
    ...
    def get_embeddings(self, options, embeddings_path, word2idx):
        ...
        elif options.emb == "onehot":
            out = self.get_onehot(word2idx)
        return out
