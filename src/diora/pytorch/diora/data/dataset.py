...
from diora.data.embeddings import UNK_TOKEN, EmbeddingsReader, get_onehot
from diora.data.preprocessing import (
    build_text_vocab,
    indexify,
    synthesize_training_data,
)
from diora.data.reading import (
    ConllReader,
    JSONLReader,
    NLIReader,
    PlainTextReader,
...
class ConsolidateDatasets(object):
    ...
    def run(self, emb):
        ...
        # embeddings = self.remap_embeddings(
        #    self.datasets, inverse_mapping_lst, master_word2idx
        # )

        if emb == "onehot":
            embeddings, _ = get_onehot(word2idx=master_word2idx)
        else:
            embeddings = self.remap_embeddings(
                self.datasets, inverse_mapping_lst, master_word2idx
            )
...
class ReaderManager(object):
    ...
    def run(
        ...
        # Added.
        # Force to use externally given word2idx and embeddings
        if (ext_word2idx is not None) and (ext_embeddings is not None):
            word2idx = ext_word2idx
            embeddings = ext_embeddings
...
class ReconstructDataset(object):
    ...
    def initialize(
        ...
        if data_type == "nli":
            reader = NLIReader.build(
                lowercase=options.lowercase, filter_length=filter_length
            )
            ...
            reader = SyntheticReader(
                nexamples=options.synthetic_nexamples,
                embedding_size=options.synthetic_embeddingsize,
                vocab_size=options.synthetic_vocabsize,
                seed=options.synthetic_seed,
                minlen=options.synthetic_minlen,
                maxlen=options.synthetic_maxlen,
        ...
        result = manager.run(
            ...
            text_path,
            embeddings_path,
            ext_word2idx=ext_word2idx,
            ext_embeddings=ext_embeddings,
        )
...
def make_batch_iterator(
    ...
    if options.reconstruct_mode in ("margin", "softmax"):
        ...
        negative_sampler = NegativeSampler(
            freq_dist=freq_dist, dist_power=options.freq_dist_power
        )
