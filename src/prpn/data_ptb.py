...
class TreeFormatCorpus(object):
    """Does not consider the name of nonterminals."""

    def __init__(self, train_file: str, dev_file: str, test_file: str):
        # Create vocabulary from train file.
        self.dictionary = Dictionary()
        self.add_words([train_file])

        self.train, self.train_sens, self.train_trees = self.tokenize(Path(train_file))
        self.valid, self.valid_sens, self.valid_trees = self.tokenize(Path(dev_file))
        self.test, self.test_sens, self.test_trees = self.tokenize(Path(test_file))

    def add_words(self, files: list[str]):
        for filename in files:
            with Path(filename).open(mode="r") as f:
                for l in f:
                    sen_tree = nltk.Tree.fromstring(l)
                    words: list[str] = ["<s>"] + sen_tree.leaves() + ["</s>"]
