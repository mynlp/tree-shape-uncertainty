...
class ParsingNetwork(nn.Module):
    def __init__(
        self, ninp, nhid, nslots=5, nlookback=1, resolution=0.1, dropout=0.4, hard=False
    ):
    ...
    def forward(self, emb, parser_state):
        ...
        emb = emb_last.transpose(0, 1).transpose(
            1, 2
        )  # bsz, ninp, ntimestep + nlookback
        ...
        if self.hard:
            memory_gate_next = (
                F.hardtanh((gate_next[:, :, None] - gate_hat) / self.resolution * 2 + 1)
                + 1
            ) / 2
        else:
        ...
        memory_gate_next = torch.cumprod(
            memory_gate_next, dim=2
        )  # bsz, ntimestep, nlookback+1
    ...
    def init_hidden(self, bsz):
        ...
        return Variable(weight.new(self.nlookback, bsz, self.ninp).zero_()), Variable(
            weight.new(bsz, self.nslots).zero_() + numpy.inf
        )
