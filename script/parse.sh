...
for split in ${splits}; do
        python -m src.train_many --dataset_key_l ${dataset_key_l} --archi_l ${archi_l} --train_random_seeds ${train_seeds} --n_parallel ${n_parallel} --use_gpu ${use_gpu} --dataset_dir ${dataset_dir} --train_dir ${train_dir} --log_dir ${log_dir} --force_update ${force_update} --debug False --gpu_ids ${gpu_ids} --only_eval True --split_to_parse ${split}
done